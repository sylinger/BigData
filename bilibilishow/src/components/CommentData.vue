...
export default {
  ...
  methods: {
    drawCharts() {
      ...
      const videoComments = this.videoData.map(item => parseInt(item.reply_data));
      ...
      const gradientColor = {
        ...
        y2: 1,
        colorStops: [
          { offset: 0, color: '#FAD961' }, // 顶部颜色
          { offset: 1, color: '#F76B1C' }, // 底部颜色
        ],
      };
        ...
        tooltip: {
          ...
          formatter: params => `
            <div style="text-align: left;">
              <b>${params.name}</b><br/>
              <span style="color: #888;">评论数: </span>${params.value}
            </div>
          `,
        },
        series: [
      ...
      const upComments = this.upData.map(item => parseInt(item.reply_data));
  ...
  mounted() {
    axios.get('/static/video_reply_data.csv').then(response => {
      Papa.parse(response.data, {
        header: true,
        complete: results => {
          this.videoData = results.data.filter(item => item.title);
          axios.get('/static/up_reply_data.csv').then(response => {
            Papa.parse(response.data, {
              header: true,
              complete: results => {
