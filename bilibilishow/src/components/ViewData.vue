...
export default {
  ...
  methods: {
    drawCharts() {
      ...
      const videoViews = this.videoData.map(item => parseInt(item.view_data));
      ...
      const gradientColor = {
        ...
        y2: 1,
        colorStops: [
          { offset: 0, color: '#A18CD1' }, // 顶部颜色
          { offset: 1, color: '#FBC2EB' }, // 底部颜色
        ],
      };
        ...
        tooltip: {
          ...
          formatter: params => `
            <div style="text-align: left;">
              <b>${params.name}</b><br/>
              <span style="color: #888;">播放量: </span>${params.value}
            </div>
          `,
        },
        series: [
      ...
      const upViews = this.upData.map(item => parseInt(item.view_data));
  ...
  mounted() {
    axios.get('/static/video_view_data.csv').then(response => {
      Papa.parse(response.data, {
        header: true,
        complete: results => {
          this.videoData = results.data.filter(item => item.title);
          axios.get('/static/up_view_data.csv').then(response => {
            Papa.parse(response.data, {
              header: true,
              complete: results => {
