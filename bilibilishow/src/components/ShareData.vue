...
export default {
  ...
  methods: {
    drawCharts() {
      ...
      const videoShares = this.videoData.map(item => parseInt(item.share_data));
      ...
      const gradientColor = {
        ...
        y2: 1,
        colorStops: [
          { offset: 0, color: '#FF9A8B' }, // 顶部颜色
          { offset: 1, color: '#FBD786' }, // 底部颜色
        ],
      };
        ...
        tooltip: {
          ...
          formatter: params => `
            <div style="text-align: left;">
              <b>${params.name}</b><br/>
              <span style="color: #888;">转发次数: </span>${params.value}
            </div>
          `,
        },
        series: [
      ...
      const upShares = this.upData.map(item => parseInt(item.share_data));

      const upBarChart = echarts.init(document.getElementById('upBar'));
      const upBarOption = {
        title: {
          text: 'Top10转发次数最多的UP主',
          left: 'center',
          top: '5%',
          textStyle: {
            fontSize: 26,
  ...
  mounted() {
    axios.get('/static/video_share_data.csv').then(response => {
      Papa.parse(response.data, {
        header: true,
        complete: results => {
          this.videoData = results.data.filter(item => item.title);
          axios.get('/static/up_share_data.csv').then(response => {
            Papa.parse(response.data, {
              header: true,
              complete: results => {
